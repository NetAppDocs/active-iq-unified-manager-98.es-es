...
[NOTE]
====
El "`WFA pack for managing Clustered Data ONTAP`" está incluido en WFA 5.1, así que no necesitas descargar este pack de la NetAppStorage Automation Store ni instalarlo por separado en tu servidor WFA como se requería antes. https://community.netapp.com/["PAQUETE WFA para gestionar ONTAP"^]
